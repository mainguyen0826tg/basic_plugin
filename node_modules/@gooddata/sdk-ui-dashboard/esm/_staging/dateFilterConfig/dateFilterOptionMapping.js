...
var VIRTUAL_PRESET_IDENTIFIER = "GDC__VIRTUAL_PRESET";
var virtualPresetBase = {
...
export function matchDateFilterToDateFilterOptionWithPreference(dateFilter, availableOptions, preferredOptionId) {
    var preferredOption = preferredOptionId
...
export function matchDateFilterToDateFilterOption(dateFilter, availableOptions) {
    ...
    if (!dateFilter) {
        var allTime = availableOptions.allTime;
        return allTime
    ...
    var matchingFilter = findDateFilterOptionByValue(dateFilter, availableOptions);
    if (matchingFilter) {
    ...
    if (dateFilter.dateFilter.type === "relative" && ((_a = dateFilter.dateFilter.to) === null || _a === void 0 ? void 0 : _a.toString()) === "-1") {
        var filterToMatch = {
            dateFilter: __assign(__assign({}, dateFilter.dateFilter), { from: Number.parseInt(dateFilter.dateFilter.from.toString(), 10) + 1, to: 0 }),
        };
        var matchingFilter_1 = findDateFilterOptionByValueAndType(filterToMatch, availableOptions, "relativePreset");
        if (matchingFilter_1) {
            return { dateFilterOption: matchingFilter_1, excludeCurrentPeriod: true };
        }
    }
...
function reconstructFormForStoredFilter(options, dateFilter) {
    if (dateFilter.dateFilter.type === "absolute") {
        var dateFilterOption = __assign(__assign({}, options.absoluteForm), { from: dateFilter.dateFilter.from.toString(), to: dateFilter.dateFilter.to.toString(), type: "absoluteForm" });
        return { dateFilterOption: dateFilterOption, excludeCurrentPeriod: false };
    }
    else {
        var dateFilterOption = __assign(__assign({}, options.relativeForm), { from: Number.parseInt(dateFilter.dateFilter.from.toString(), 10), to: Number.parseInt(dateFilter.dateFilter.to.toString(), 10), granularity: dateFilter.dateFilter.granularity, type: "relativeForm" });
        return { dateFilterOption: dateFilterOption, excludeCurrentPeriod: false };
    }
}
...
function findDateFilterOptionById(id, dateFilterOptions) {
    return flattenDateFilterOptions(dateFilterOptions).find(function (option) { return option.localIdentifier === id; });
}
...
function findDateFilterOptionByValueAndType(dateFilter, dateFilterOptions, type) {
    return flattenDateFilterOptions(dateFilterOptions)
        .filter(function (option) { return option.type === type; })
        .find(filterMatchesData(dateFilter));
}
var filterMatchesData = function (dateFilter) { return function (filter) {
    var _a, _b;
    if (!filter) {
        return false;
    }
    var data = dateFilter.dateFilter;
...
function createVirtualPresetForStoredFilter(dateFilter) {
    if (!dateFilter) {
        var dateFilterOption = __assign(__assign({}, virtualPresetBase), { type: "allTime" });
        return { dateFilterOption: dateFilterOption, excludeCurrentPeriod: false };
    }
    var _a = dateFilter.dateFilter, type = _a.type, from = _a.from, to = _a.to, granularity = _a.granularity;
    if (type === "absolute") {
        var dateFilterOption = __assign(__assign({}, virtualPresetBase), { from: from.toString(), to: to.toString(), type: "absolutePreset" });
        return { dateFilterOption: dateFilterOption, excludeCurrentPeriod: false };
    }
    else {
        var dateFilterOption = __assign(__assign({}, virtualPresetBase), { from: Number.parseInt(from.toString(), 10), to: Number.parseInt(to.toString(), 10), granularity: granularity, type: "relativePreset" });
        return { dateFilterOption: dateFilterOption, excludeCurrentPeriod: false };
    }
}
